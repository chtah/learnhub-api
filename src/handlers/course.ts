    ...
    async (req, res) => {
      ...
      try {
        ...
        const mappedMenu = course.menu.map((menuItem) => {
          const { id, menu_title, submenu } = menuItem;
          const mappedSubmenu = submenu.map((submenuItem) => {
            const { id, submenu_title, topic } = submenuItem;
            const mappedTopics = topic.map((topicItem) => {
              const { id, topic_title, topic_description, topic_url } =
                topicItem;
              return { id, topic_title, topic_description, topic_url };
            });

            return { id, submenu_title, topic: mappedTopics };
          });
          return { id, menu_title, submenu: mappedSubmenu };
        });
